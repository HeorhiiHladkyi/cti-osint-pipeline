...
- **Встановлення IP цілі** дає публічно-резолвний IP + ASN. Якщо ціль за реверс-проксі
  ...
  Пайплайн робить **базовий origin-hunting**: резолвить субдомени з crt.sh і підсвічує ті,
  що йдуть повз CDN на ймовірний origin-сервер. Це знаходить лише origin, який «світиться»
  через неправильно налаштовані субдомени; глибше розкриття (історичний DNS,
  Shodan favicon/cert-hash) потребує платних джерел і не входить у поточний обсяг.
