...
crt.sh, RDAP/ASN, URLScan, блокчейн). Платні джерела (VirusTotal, AbuseIPDB, Shodan, OTX)
підключаються автоматично за наявності ключа в `.env`, інакше пропускаються з `WARNING`.

Окрім мережевих IoC, підтримуються **крипто-гаманці (BTC/ETH)** як індикатори: пайплайн
збагачує їх on-chain даними (баланс, кількість транзакцій, активність, контрагенти) —
це моніторинг крипто-операцій адреси, у стилі кейсу Bybit/Lazarus.
...
185.220.101.44
```
Типи визначаються автоматично: IPv4, домен, URL, MD5/SHA256, email, **крипто-гаманець
(BTC/ETH)**. Приклад із гаманцями — [`examples/crypto-iocs.txt`](examples/crypto-iocs.txt)
(`python run.py -i examples/crypto-iocs.txt`). Демонстрація вердикту `critical` на
відомих шкідливих адресах (WannaCry, Tornado Cash) —
[`examples/crypto-malicious.txt`](examples/crypto-malicious.txt).
...
| blockchain.info | ні | **BTC-гаманець**: баланс, транзакції, отримано/надіслано, контрагенти |
| Blockscout (ETH) | ні | **ETH-гаманець**: баланс, к-сть транзакцій, активність, контрагенти |
| Вбудована база загроз | ні | **репутація гаманця**: OFAC-санкції, ransomware-адреси (вердикт offline) |
| Chainabuse | так (опц.) | живі звіти про скам/зловживання для крипто-адрес |
...
- **Крипто-гаманці**: безключове блокчейн-збагачення дає *контекст* операцій (баланс,
  транзакції, контрагенти). Вердикт загрози ставиться за **вбудованою базою відомих
  шкідливих адрес** (OFAC-санкції, ransomware) — вона працює offline, але охоплює лише
  курований перелік. Повне покриття (нові міксери, скам, кластеризація) потребує платних
  feed'ів — Chainabuse (підключається ключем), Arkham, Chainalysis, TRM. Прямого зв'язку
  «IP → транзакція» в блокчейні немає (мережа не зберігає IP); крипто і мережеві IoC —
  окремі типи вузлів.
