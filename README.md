...
(`python run.py -i examples/crypto-iocs.txt`).
...
- **Крипто-гаманці**: безключове блокчейн-збагачення дає *контекст* операцій (баланс,
  транзакції, контрагенти), але **не вердикт загрози** — мітки «міксер / санкційна адреса /
  ransomware» потребують платних feed'ів (Arkham, Chainalysis, TRM). Тому гаманець без
  зовнішнього сигналу лишається `low`. Прямого зв'язку «IP → транзакція» в блокчейні немає
  (мережа не зберігає IP); крипто і мережеві IoC — окремі типи вузлів.
